...
describe('it should work', () => {
	...
	test('it should return a 0 breakpoint if no changes are observed', () => {
		expect(proxy.breakpoint()).toBe(0);
	});
...
describe('it should support mutations', () => {
	...
	test('it should change the length of an array on undo', () => {
		proxy.chunks = [];
		proxy.chunks.push([1,2,3,4,5]);
...
describe('it should support rollback', () => {
	...
	describe('it should rollback on error', () => {

		test('it should undo a single change on error', () => {
			try {
				proxy.foobar = 'this goes away';
				expect(proxy.foobar).toEqual('this goes away');
				throw new Error('I make Error!');
			} catch(e) {
				proxy.undo();
			}
			expect(proxy.foobar).toBe(undefined);
		});

		test('it should rollback a single change on error', () => {
			try {
				proxy.foobar = 'this goes away';
				expect(proxy.foobar).toEqual('this goes away');
				throw new Error('I make Error!');
		...
		test('it should rollback multiple changes on error', () => {
			try {
				proxy.foobar = 'this goes away';
				proxy.data = [1,8,4,2,6,5,3,7];
				proxy.data.sort((a,b) => ((a-b)));
				expect(proxy.foobar).toEqual('this goes away');
				expect(proxy.data).toEqual(expect.objectContaining([1,2,3,4,5,6,7,8]));
				throw new Error('I make Error!');
			} catch(e) {
				proxy.rollback();
			}
		...
		test('it should rollback to a breakpoint on error', () => {
			proxy.chunks = [];
			const breakpoints = [];
			const buildBreakpoints = () => {
				for (;;) {
					breakpoints.push({index: proxy.breakpoint(), length: proxy.chunks.length});
					const random = Math.random() * 4;
					const chunk = (new Array(Math.ceil(random))).map(() => Number(random * 10).toFixed(3));
					proxy.chunks.push(chunk);
					if (random > 3.5 && breakpoints.length > 3) {
						throw new Error('I make Error!');
					}
				}
			};
			try {
				...
				if (err.message !== 'I make Error!') {
					throw err;
				}
				const breakpoint = breakpoints.pop();
				expect(breakpoint.index).toBe(proxy.breakpoint() - 1);
				proxy.rollback(breakpoint.index);
				expect(proxy.chunks.length).toBe(breakpoint.length);
				expect(data.chunks.length).toBe(proxy.chunks.length);
			}
		});
